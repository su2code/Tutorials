...
# reference temperature for enthalpy computations
Tref = 298.15
...
# ################################################################## #
# returns laminar flamespeed as function of the enthalpy deficiency  #
# dh = 0 returns S_L at Tu=673K (methane-air, phi=0.5, P=5 bar)      #
# ################################################################## #
def SL(val_dh): 
  dh = val_dh/1000.0  
  dh = max(-500.0, min(dh, 200.0))
  S_L = max(0.05, 0.2327 + 1.177278e-3 * dh + 2.234098e-6*dh*dh + 1.56795e-9*dh*dh*dh)
  #print("dH = ", dh, ", S_L =", S_L)
  return S_L
...
# ################################################################## #
# loop over all vertices on a boundary and impose wall temperature   #
# ################################################################## #
def ApplyWallTemperature(SU2Driver, marker_ids):
  ...
  for marker_id in marker_ids:
    ...
    for i_vertex in range(SU2Driver.GetNumberMarkerNodes(marker_id)):
      i_point = SU2Driver.GetMarkerNode(marker_id, i_vertex)
      coord = allCoords.Get(i_point)
      Tw = 673 + 500.0 * (coord[1]-0.0125)/0.025  
      SU2Driver.SetMarkerCustomTemperature(marker_id, i_vertex, Tw)
...
def zimont(SU2Driver, iPoint, nDim):
    ...
    # assume linear relation between progress variable and heat of combustion
    iTEMPERATURE = primindex.get("TEMPERATURE")
    T = SU2Driver.Primitives()(iPoint,iTEMPERATURE)

    C = SU2Driver.Solution(iSPECIESSOLVER)(iPoint,0)
    delta_H = C * 0.0284*50.5*1.0e6
    # adiabatic temperature when no heat losses:
    T_adiabatic = (delta_H/cp_u) + Tu
    # heat loss
    dh = cp_u*(T - T_adiabatic)
    #print("T_adiabatic=",T_adiabatic, ", actual T = ",T, ", heat los = dh=",dh )
    # correction to SL using simple correlation for methane-air 
    # no heat loss is original S_L
    # heat loss gives the new S_L
    
    Ut = SL(dh) * (1.0 + (0.46/Le) * np.power(Re,0.25) * np.power(up/SL(dh),0.3) * np.power(Pu,0.2) )
...
def main():
  ...
  # Get the ID of the markers where the temperature is applied.
  ...
  print("marker names where the temperature boundary condition will be applied:", marker_names)
  print("marker IDs where the temperature boundary condition will be applied:", marker_ids)
  ...
  for inner_iter in range(N):
    ...
    # impose wall temperature 
    ApplyWallTemperature(driver,marker_ids)
    ...
    for i_node in range(driver.GetNumberNodes() - driver.GetNumberHaloNodes()):
      ...
      iPV = 0
      Source.Set(i_node, iPV, S)
      iENTH = 3
      Source_h.Set(i_node, iENTH, 1.0* 0.0284*50.5*1e6*S)
